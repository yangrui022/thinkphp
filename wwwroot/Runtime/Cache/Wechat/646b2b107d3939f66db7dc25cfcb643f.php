...
<div class="main">
    ...
    <div class="container-fluid" id="content_list">
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="row noticeList">
                <a href="<?php echo U('detail?id='.$vo['id']);?>">
                    <div class="col-xs-2">
                        <img class="img-rounded img-responsive" src="<?php echo (get_cover($vo["cover_id"],'path')); ?>" />
                    </div>
                    <div class="col-xs-10">
                        <p class="title"><?php echo ($vo["title"]); ?></p>
                        <p class="intro"><?php echo ($vo["description"]); ?></p>
                        <p class="info">浏览:<?php echo ($vo["view"]); ?> <span class="pull-right">截止时间：<?php echo date('Y-m-d H:i:s',$vo['deadline']);?></span> </p>
                    </div>
                </a>
    ...
    <script type="application/javascript">
        var p = 1;
        var url = '/wechat.php?s=/Active/index';
        var inner_url = '/wechat.php?s=/Active/detail';
        $(function(){
                                ...
                                html += '<div class="row noticeList">\
                                            <a href="'+inner_url+'/id/'+inner['id']+'.html">\
                                                ...
                                                <div class="col-xs-10">\
                                                    ...
                                                    <p class="info">浏览: '+inner['view']+' <span class="pull-right">'+'截止时间：'+getLocalTime(inner['deadline'])+'</span> </p>\
                                                </div>\
                                            </a>\
                                        </div>';
                            }
